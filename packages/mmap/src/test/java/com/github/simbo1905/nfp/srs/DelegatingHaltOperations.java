...
class DelegatingHaltOperations extends AbstractDelegatingFileOperations {
  ...
  public DelegatingHaltOperations(FileOperations delegate, int haltAtOperation) {
    ...
    logger.log(
        ...
        () ->
            String.format(
                "Created DelegatingHaltOperations with haltAtOperation=%d", haltAtOperation));
  }
  ...
  protected void handleTargetOperation() {
    ...
    logger.log(
        ...
        () ->
            String.format(
                "HALTING at operation %d - all subsequent operations will be silently ignored",
                operationCount));
  }
  ...
  public int read(byte[] b) throws IOException {
    ...
    if (halted) {
      logger.log(
          Level.FINEST, () -> String.format("HALTED: read(%d bytes) returning -1", b.length));
      return -1;
    }
