...
record RecordHeader(
  ...
  RecordHeader(long dataPointer, int dataLength, int dataCapacity) {
    ...
    if (dataLength != dataCapacity) {
      System.err.println(String.format("RecordHeader CONSTRUCTOR: dataPointer=%d, dataLength=%d, dataCapacity=%d", dataPointer, dataLength, dataCapacity));
    }
  }
  ...
  /// Does NOT include file position (dataPointer) as that causes unnecessary churn when records move.
  /// File position is transient metadata - including it would require recomputing CRCs on every move.
  ...
  private static long computeHeaderCrc(int dataLength, int dataCapacity) {
    ...
    final int HEADER_DATA_SIZE = Integer.BYTES + Integer.BYTES + Short.BYTES; // 4 + 4 + 2 = 10 bytes
    ...
    buffer.putShort((short) 0); // 2 bytes placeholder for keyLength salt (actual key length added separately)
  ...
  static RecordHeader withDataPointerAndCapacity(RecordHeader original, long newDataPointer, int newDataCapacity) {
    Objects.requireNonNull(original, "original RecordHeader cannot be null");
    return new RecordHeader(newDataPointer, original.dataLength, newDataCapacity, original.indexPosition, 
        computeHeaderCrc(original.dataLength, newDataCapacity));
  }

  /// Creates a new RecordHeader with updated data pointer, keeping other fields unchanged.
  /// Used for atomic updates when records move but data size stays the same.
  /// Preserves original CRC since no data fields changed.
  static RecordHeader withDataPointer(RecordHeader original, long newDataPointer) {
    Objects.requireNonNull(original, "original RecordHeader cannot be null");
    return new RecordHeader(newDataPointer, original.dataLength, original.dataCapacity, original.indexPosition, original.crc32);
  }
  ...
  static RecordHeader withDataCapacity(RecordHeader original, int newDataCapacity) {
    Objects.requireNonNull(original, "original RecordHeader cannot be null");
    return new RecordHeader(original.dataPointer, original.dataLength, newDataCapacity, original.indexPosition,
        computeHeaderCrc(original.dataLength, newDataCapacity));
  }
  ...
  static RecordHeader withDataCount(RecordHeader original, int newDataCount) {
    Objects.requireNonNull(original, "original RecordHeader cannot be null");
    return new RecordHeader(original.dataPointer, newDataCount, original.dataCapacity, original.indexPosition,
  ...
  static RecordHeader withIndexPosition(RecordHeader original, int newIndexPosition) {
    Objects.requireNonNull(original, "original RecordHeader cannot be null");
    return new RecordHeader(original.dataPointer, original.dataLength, original.dataCapacity, newIndexPosition, original.crc32);
  }
  ...
  RecordHeader split(boolean disableCrc32, int padding) {
    ...
    RecordHeader newRecord = new RecordHeader(newFp, freeSpace, freeSpace);
  ...
  /// - split(): divide [record + free] → [record] + [free header]
  /// - incrementDataCapacity(): merge [record + free] → [larger record]
  ...
  static long computeCrc32(byte[] data, int offset, int length) {
    java.util.zip.CRC32 crc = new java.util.zip.CRC32();
  ...
  static String formatForLog(RecordHeader header, byte[] preview) {
    return String.format(
        ...
        header.dataPointer(), header.dataLength(), header.dataCapacity(), header.crc32(),
  ...
  static long computeKeyCrc(byte[] key) {
    ...
    buffer.put(key);
    return computeCrc32(buffer.array(), 0, buffer.array().length);
  }
}
