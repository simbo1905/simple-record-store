...
public class FileRecordStoreExceptionHandlingTest extends JulLoggingConfig {
    ...
    private int discoverIndexManipulationScenarioOperationCount(Path filePath) throws Exception {
        ...
        // Replace with counting delegate that never throws
        java.io.RandomAccessFile raf = new java.io.RandomAccessFile(countingStore.getFilePath().toFile(), "rw");
        RandomAccessFile directOps = new RandomAccessFile(raf);
        DelegatingExceptionOperations countingOps = new DelegatingExceptionOperations(directOps, Integer.MAX_VALUE);
    ...
    private void testReadScenarioWithException(int throwAt, String description) throws Exception {
        ...
        java.io.RandomAccessFile raf = new java.io.RandomAccessFile(testStore.getFilePath().toFile(), "r");
        RandomAccessFile directOps = new RandomAccessFile(raf);
  ...
  /// Helper method for max key length scenario
    private void testMaxKeyLengthScenario(int throwAt) throws Exception {
        ...
        // Replace file operations with exception injection
        java.io.RandomAccessFile raf = new java.io.RandomAccessFile(testStore.getFilePath().toFile(), "rw");
        RandomAccessFile directOps = new RandomAccessFile(raf);
      testStore.fileOperations = new DelegatingExceptionOperations(directOps, throwAt);
            ...
            for (byte[] key : keys) {
                ...
                try {
                    ...
                    throw new AssertionError(description +
                        ": Failed to read data for key " + Base64.getEncoder().encodeToString(key) + ": " + e.getMessage(), e);
                }
    
                if (data == null) {
                    throw new AssertionError(description +
                        ": Null data for key " + Base64.getEncoder().encodeToString(key));
                }
    
                if (data.length == 0) {
                    throw new AssertionError(description +
                        ": Empty data for key " + Base64.getEncoder().encodeToString(key));
                }
                ...
                if (expectedValues != null) {
                    ...
                    if (expected == null) {
                        throw new AssertionError(description +
                            ": Unexpected key found " + Base64.getEncoder().encodeToString(key));
                    }
