...
public class HeaderExpansionTest extends JulLoggingConfig {
  ...
  public void testHeaderExpansionWithSmallBlocks() throws Exception {
    ...
    try {
      ...
      try (var store = new FileRecordStoreBuilder()
               .path(tempFile)
        ...
        logger.log(Level.FINE, "Store created with 2 preallocated records, 1KB blocks, 1MB expansion");
        ...
        Assert.assertArrayEquals("Key1 data should be intact", "data1".getBytes(), store.readRecordData(key1));
        Assert.assertArrayEquals("Key2 data should be intact", "data2".getBytes(), store.readRecordData(key2));
        Assert.assertArrayEquals("Key3 data should be intact", "data3".getBytes(), store.readRecordData(key3));
        ...
        Assert.assertArrayEquals("Key10 data should be intact", "data10".getBytes(), store.readRecordData(key10));
  ...
  public void testRecordMovementDuringExpansion() throws Exception {
    ...
    try {
      ...
      try (var store = new FileRecordStoreBuilder()
        ...
        logger.log(Level.FINE, "Store created with 1 preallocated record, 1KB blocks, 1MB expansion");
        ...
        Assert.assertArrayEquals("Key1 data should be intact after movement", "first record data".getBytes(), store.readRecordData(key1));
        Assert.assertArrayEquals("Key2 data should be intact", "second record data".getBytes(), store.readRecordData(key2));
        ...
        logger.log(Level.FINE, "Inserting large record (1KB data) - should trigger complex movement");
        ...
        Assert.assertArrayEquals("Key1 data should still be intact", "first record data".getBytes(), store.readRecordData(key1));
        Assert.assertArrayEquals("Key2 data should still be intact", "second record data".getBytes(), store.readRecordData(key2));
        Assert.assertArrayEquals("Key3 large data should be intact", largeData, store.readRecordData(key3));
  ...
  public void testHeaderExpansionWithUpdates() throws Exception {
    ...
    try {
      try (var store = new FileRecordStoreBuilder()
        ...
        logger.log(Level.FINE, "Store created - will fill pre-allocated space then force expansion with updates");
        ...
        logger.log(Level.FINE, "Updating record 1 with larger data (512 bytes) - should trigger expansion");
        store.updateRecord(key1, largeUpdateData);
        
        // Verify the update worked
        Assert.assertArrayEquals("Large update should be intact", largeUpdateData, store.readRecordData(key1));
        
        logger.log(Level.FINE, "Large update successful, now inserting new record to force further expansion");
        ...
        Assert.assertArrayEquals("Large update should still be intact", largeUpdateData, store.readRecordData(key1));
        
        byte[] key2 = new byte[32];
        for (int j = 0; j < 32; j++) {
          key2[j] = (byte) (j + 2);
        }
        Assert.assertArrayEquals("Record 2 should be intact", "initial-data-2".getBytes(), store.readRecordData(key2));
        
        Assert.assertArrayEquals("Record 4 should be intact", "fourth-record-data".getBytes(), store.readRecordData(key4));
