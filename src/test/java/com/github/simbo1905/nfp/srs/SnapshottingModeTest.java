...
public class SnapshottingModeTest extends JulLoggingConfig {
  ...
  public void testInPlaceUpdateToggle() throws Exception {
    try (FileRecordStore store = 
      ...
      Assert.assertTrue("In-place updates should be enabled by default", 
          store.isAllowInPlaceUpdates());
      
      // Test toggling off
      store.setAllowInPlaceUpdates(false);
      Assert.assertFalse("In-place updates should be disabled", 
          store.isAllowInPlaceUpdates());
      
      // Test toggling back on
      store.setAllowInPlaceUpdates(true);
      Assert.assertTrue("In-place updates should be re-enabled", 
          store.isAllowInPlaceUpdates());
    }
  }
  ...
  public void testHeaderExpansionToggle() throws Exception {
    try (FileRecordStore store = 
      ...
      // Verify default state
      Assert.assertTrue("Header expansion should be enabled by default", 
          store.isAllowHeaderExpansion());
      
      // Test toggling off
      store.setAllowHeaderExpansion(false);
      Assert.assertFalse("Header expansion should be disabled", 
          store.isAllowHeaderExpansion());
      
      // Test toggling back on
      store.setAllowHeaderExpansion(true);
      Assert.assertTrue("Header expansion should be re-enabled", 
          store.isAllowHeaderExpansion());
    }
  }
