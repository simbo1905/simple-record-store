...
/**
 * Test to demonstrate FileRecordStore constructor bugs when opening existing files.
 * 
 * Bug Report Summary:
 * 1. indexEntryLength calculation changed, breaking backward compatibility
 * 2. Resource leak when header validation fails (RandomAccessFile not closed)
 * 3. memIndex sized with unvalidated header values (OOM risk)
 * 4. dataStartPtr incorrectly initialized for existing files
 */
public class FileRecordStoreConstructorExistingFileBugTest extends JulLoggingConfig {
    ...
    public void demonstrateMemIndexSizingWithUnvalidatedData() throws Exception {
        ...
        try {
            ...
            try (FileRecordStore store = new FileRecordStore.Builder()
                ...
                logger.log(Level.FINE, "Got exception (could be good if it prevents OOM): " + e.getMessage());
            }
    ...
    public void demonstrateDataStartPtrInitializationBug() throws Exception {
        ...
        try {
            ...
            // Reopen with different preallocatedRecords - should use file's dataStartPtr, not preallocated value
